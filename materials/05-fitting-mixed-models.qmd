...
In this dataset, we want to treat `Subject` as a random effect, which means fitting a mixed effects model. Why `Subject`? There are two things at play here that make us want to treat this as a random effect:
...
::: {.panel-tabset group="language"}
## R
```{r plot_random_intercepts_facets}
...
::: {.panel-tabset group="language"}
## R
```{r fit_random_intercept_slope_model}
...
::: {.panel-tabset group="language"}
## R
```{r compare_explicit_implicit_intercepts, results='hide'}
...
If that's a bit more complicated than you're interested in, don't worry. You don't need to understand all that maths to be able to use a mixed effects model. It boils down to the same thing: that random effects are a set of coefficients with some variance, and we make assumptions about their distribution(s).
...
::: {.panel-tabset group="language"}
## R
```{r load_irrigation_data, message=FALSE}
...
::: {.panel-tabset group="language"}
## R
```{r plot_irrigation_model_points}
...
::: {.panel-tabset group="language"}
## R
```{r plot_irrigation_observed_predicted}
ggplot(augment(lme_yield), aes(x = irrigation, y = yield, colour = variety)) +
...
::: {.panel-tabset group="language"}
## R
```{r load_dragons_data, message=FALSE}
...
::: {.panel-tabset group="language"}
## R
```{r plot_dragons_wingspan_scales}
...
It would be perfectly allowable for you to fit all four of these random effects if you wanted to. The syntax to include them all would be `(1 + wingspan*scales|mountain)`, or written out in full, `(1 + wingspan + scales + wingspan:scales|mountain)`.
...
::: {.panel-tabset group="language"}
## R
```{r plot_dragons_mixed_model_facets}
...
::: {.panel-tabset group="language"}
## R
```{r plot_dragons_mixed_model_global}
