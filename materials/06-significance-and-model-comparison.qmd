...
![Two ways to use likelihood ratio tests](images/LRT_schematic.png){width=70%}
...
::: {.panel-tabset group="language"}
## R
```{r compare_fixed_effect_days_lrt}
...
::: {.panel-tabset group="language"}
## R
```{r fit_sleepstudy_with_lmertest}
library(lmerTest)
...
```{r fit_models_for_bootstrap_fixed_effect}
#| echo: false
#| message: false
#| results: hide
#| warning: false

# Note: we're refitting with lme4 as there is a bug in 
# the current version of lmerTest that conflicts with pbkrtest
lme_sleep_random <- lme4::lmer(Reaction ~ 1 + (1 + Days|Subject),
                   data = sleepstudy)
lme_sleep <- lme4::lmer(Reaction ~ Days + (1 + Days|Subject),
                   data = sleepstudy)
```

```{r bootstrap_fixed_effect}
#| warning: false
...
```{r fit_models_for_bootstrap_random_effect}
#| echo: false
#| message: false
#| results: hide
#| warning: false
...
# Note: we're refitting with lme4 as there is a bug in 
# the current version of lmerTest that conflicts with pbkrtest
lme_sleep_intercepts <- lme4::lmer(Reaction ~ Days + (1|Subject),
                   data = sleepstudy)
```

```{r bootstrap_random_effect}
#| warning: false
...
::: {.panel-tabset group="language"}
## R
```{r load_and_fit_dragons, message=FALSE}
...
::: {.panel-tabset group="language"}
## R
```{r compare_dragons_model_to_null}
...
::: {.panel-tabset group="language"}
## R
```{r compare_dragons_interaction_lrt}
...
```{r compare_dragons_main_effects_lrt}
...
```{r fit_dragons_models_for_bootstrap}
#| echo: false
#| message: false
#| results: hide
#| warning: false
...
# Note: we're refitting with lme4 as there is a bug in 
# the current version of lmerTest that conflicts with pbkrtest
lme_dragons <- lme4::lmer(intelligence ~ wingspan*scales + 
                          (1 + wingspan|mountain), 
                          data = dragons)
lme_dragons_dropx <- lme4::lmer(intelligence ~ wingspan + scales + 
                                (1 + wingspan|mountain), 
                                data = dragons)
lme_dragons_dropscale <- lme4::lmer(intelligence ~ wingspan + 
                                    (1 + wingspan|mountain), 
                                    data = dragons)
lme_dragons_dropwing <- lme4::lmer(intelligence ~ scales + 
                                   (1 + wingspan|mountain),
                                   data = dragons)
```

```{r bootstrap_dragons_fixed_effects}
#| warning: false
...
```{r fit_dragons_random_effect_models}
...
```{r compare_dragons_random_effects_lrt}
...
```{r fit_dragons_random_models_for_bootstrap}
#| echo: false
#| message: false
#| results: hide
#| warning: false
...
# Note: we're refitting with lme4 as there is a bug in 
# the current version of lmerTest that conflicts with pbkrtest
lme_dragons_dropslope <- lme4::lmer(intelligence ~ wingspan*scales + 
                                    (1|mountain),
                                    data = dragons)
lme_dragons_dropint <- lme4::lmer(intelligence ~ wingspan*scales + 
                                  (0 + wingspan|mountain),
                                  data = dragons)
```

```{r bootstrap_dragons_random_effects}
#| warning: false
...
::: {.panel-tabset group="language"}
## R
```{r load_and_fit_irrigation, message=FALSE}
